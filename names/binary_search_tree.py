...
class BSTNode:
    ...
    def insert(self, value):
        if value >= self.value:
            if (
                self.right == None
            ):  # value is greater than or equal to the node currently referred to as self and there is not another node that fits that description.
                self.right = BSTNode(value)
            else:
                self.right.insert(
                    value
                )  # rerun this method with the correct child node
        else:
            if (
                self.left == None
            ):  # value is less than the node currently referred to as self and there is not another node that fits that description.
    ...
    def in_order_print(self, node):
        if (
            node.left
        ):  # if the current node has a left child node, run the method with that node as the argument
            node.in_order_print(node.left)
        print(node.value)  # print the current node
        if (
            node.right
        ):  # run this method with the right child of the current node if it exists
